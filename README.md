# 앞서 해당 레포지토리의 정보
해당 레포지토리는 현재 정식 운영중인 끄투플러스의 레포지토리와 미세하게 다른 부분이 있습니다.
차후 수정시 추가 예정입니다.
...
본 소스코드는 2020.03.07 일자로 업데이트 되었습니다.
AGPL 적용에 따라 끄투플러스 소스코드 라이센스가 AGPL로 업데이트 되었음을 알립니다.
끄투플러스는 본 서버만 구동하며 본 소스코드와 상호 통신하는 소프트웨어는 일절 없음을 알립니다.
